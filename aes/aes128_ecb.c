...
// public functions
int aes128_ecb_encrypt(const uint8_t *input, size_t inputSize, const uint8_t *key, size_t keySize, uint8_t *output, size_t outputSize);
int aes128_ecb_decrypt(const uint8_t *input, size_t inputSize, const uint8_t *key, size_t keySize, uint8_t *output, size_t outputSize);
...
/* Encrypts 'input' with 'key' according to AES128 in ECB mode
...
int aes128_ecb_encrypt(const uint8_t *input, size_t inputSize, const uint8_t *key, size_t keySize, uint8_t *output, size_t outputSize)
{
	if (keySize != 16 || inputSize > outputSize || outputSize % 16 != 0 || !input || !key || !output) {
	...
	for (int outputOffset = 0; outputOffset < outputSize; outputOffset += 16) {
		...
		sub_bytes(&output[outputOffset]);
		shift_row(&output[outputOffset]);
		add_round_key(&output[outputOffset], expandedKey, 10);
	}
...
/* Decrypts 'input' with 'key' according to AES128 in ECB mode
 * and saves it to 'output'
 *
 * const uint8_t *input: arbitrary byte-array
 * size_t inputSize:     size of 'input' in byte
...
/* Expands the key to the 176 byte expandedKey according to AES specifications
 *
