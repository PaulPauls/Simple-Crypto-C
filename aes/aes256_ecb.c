 ...
 * 
 * Input: const uint8_t *input	byte-array of arbitrary content to encrypt
 ...
 *
 * Process: [1] check for correct parameters
...
void aes256_ecb_encrypt(const uint8_t *input, size_t inputSize, const uint8_t *key, size_t keySize, uint8_t *state, size_t stateSize)
{
	...
	uint8_t mixMultMatrix[]	= {0x02, 0x03, 0x01, 0x01,
							   ...
							   0x01, 0x01, 0x02, 0x03,
							   0x03, 0x01, 0x01, 0x02};
	uint8_t expandedKey[240]; // 240 = 16 * 15 = blocksize * (numberOfRounds+1)
...
void key_expansion(const uint8_t *key, uint8_t *expandedKey, const uint8_t *sbox)
{
	...
	for(round = 8; round < (240 / 4); round++) {
		if(round % 4 == 0) {
			if(round % 8 == 0) {
				// RoundKey = sub_word(rot_word(expanded_key(round - 1) * 4)))
				//			XOR rcon((round / 8) - 1)
				...
				// RoundKey = sub_word(expanded_key(round - 1) * 4))
				//			XOR expanded_key((round - 8) * 4)
				expandedKey[(4 * round) + 0] =
						  sbox[expandedKey[(4 * (round - 1)) + 0]]
						^ expandedKey[(4 * (round - 8)) + 0];
			...
			// RoundKey = expanded_key((round - 1) * 4)
			//			XOR expanded_key((round - 8) * 4)
			expandedKey[(4 * round) + 0] =
