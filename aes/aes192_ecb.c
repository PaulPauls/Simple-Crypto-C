...
/* Encrypts input with key to state with AES192 in ECB Mode
 *
 * Input:
 ...
 *	size_t keySize		size of key in bytes. Has to be 24byte/192bit
...
int aes192_ecb_encrypt(const uint8_t *input, size_t inputSize,
	const uint8_t *key, size_t keySize, uint8_t *state, size_t stateSize)
{
	if (keySize != 24 || inputSize > stateSize || stateSize % 16 != 0 ||
		!input || !key || !state) {
		fprintf(stderr, "Error: aes192_ecb_encrypt(...) was called with one of the following errors:\n \
			\t Size of key was not 192 bit\n \
	...
	static const uint8_t mixMultMatrix[]	= {0x02, 0x03, 0x01, 0x01,
		...
		0x01, 0x01, 0x02, 0x03,
		0x03, 0x01, 0x01, 0x02};
	uint8_t expandedKey[208]; // 208 = 16 * 13 = blocksize * (NrOfRounds+1)
...
/* Decrypts input with key to state with AES192 in ECB Mode
 *
 * Input:
 *	const uint8_t *input	byte-array encrypted with AES192 to decrypt
...
int aes192_ecb_decrypt(const uint8_t *input, size_t inputSize,
	const uint8_t *key, size_t keySize, uint8_t *state, size_t stateSize)
{
	if (keySize != 24 || inputSize > stateSize || inputSize % 16 != 0 ||
		!input || !key || !state) {
		fprintf(stderr, "Error: aes192_ecb_decrypt(...) was called with one of the following errors:\n \
			\t Size of key was not 192 bit\n \
			\t OR inputarray was larger than the statearray\n \
...
/* Input: const uint8_t *key	byte-array with original 192bit key
 *		  uint8_t *expandedKey	arbitrary byte-array with 208 bytes
 *		  const uint8_t *sbox	byte-array AES sbox
 *
 * Output: expands the key to the 208 key values according to AES specification
 */
static void key_expansion(const uint8_t *key, uint8_t *expandedKey,
	const uint8_t *sbox)
{
