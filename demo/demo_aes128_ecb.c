...
#include "aes128_ecb.h" // aes128_ecb_encrypt(...), aes128_ecb_decrypt(...)
...
/* Demo of the functions defined in aes128_ecb.c. Key and plaintexts are test
...
int main()
{
	...
	if (aes128_ecb_encrypt(plaintext, sizeof(plaintext), key, sizeof(key), ciphertext, sizeof(ciphertext)) < 0) {
		fprintf(stderr, "Error when calling aes128_ecb_encrypt(...)");
		return EXIT_FAILURE;
	}

	if (aes128_ecb_decrypt(ciphertext, sizeof(ciphertext), key, sizeof(key), decryptedCiphertext, sizeof(decryptedCiphertext)) < 0) {
		fprintf(stderr, "Error when calling aes128_ecb_decrypt(...)");
		return EXIT_FAILURE;
	}
